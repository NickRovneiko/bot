...
def check_buy_option(varian=False, quote=False):
    strike = int(((quote['close'] // 50)+1) * 50)
    ...
    try:
        # ETH-18SEP21-3600-P
        result = api.get_quote(exchange=varian.exchange, pair=f'ETH-{expiration_2day}-{strike}-P')
        ic()
        ic(result)
        expiration = timezone.now() + timedelta(days=2)
    except:
        result = api.get_quote(exchange=varian.exchange, pair=f'ETH-{expiration_1day}-{strike}-P')
        ic()
        ic(result)
        expiration = timezone.now() + timedelta(days=1)

    if result['ask'] * quote['close'] < 80:
        # виртуальная покупка опицона
        price = result['ask'] * quote['close']
    else:
        return

    Options(varian=varian.name,
            buy_price=price,
            strike=strike,
            expiration=expiration,
            amount=1).save()

    ic(Options.objects.all)
    exit()

    return


def try_buy(positions=False, quote=False):
    amount_base = g.balance / quote.close

    positions = g.df_positions.append({'varian': g.varian.name,
                                            'buy_price': g.quote.close,
                                            'opened': g.quote.timestamp,
                                            'amount_base': amount_base,
                                            'active': 1},
